...
function velocity_quiz_user($type = 'quiz') {
    ...
    if(empty($post_list)){
        ...
        foreach ($post_list as $post) {
            ...
            $quizjawab = $wpdb->get_results("SELECT * FROM $table_name WHERE vq_type = 'quiz' and post_id = $post_id and user_id = $user_id");        
            $essayjawab = $wpdb->get_results("SELECT * FROM $table_name WHERE vq_type = 'essay' and post_id = $post_id and user_id = $user_id");  
            $hasil_ujian = '';
            if($quizjawab){
                $postdetail = $quizjawab[0]->vq_detail;
                $detail = json_decode($postdetail);
                $post_date = $quizjawab[0]->vq_date;
                $hasil_ujian = '<td class="text-success fw-bold">'.$detail->nilai.'</td>';
            } if($essayjawab){
                $postdetail = $essayjawab[0]->vq_detail;
                $detail = json_decode($postdetail);
                $hasil_ujian = '<td class="text-success fw-bold"><pre>'.print_r($detail,1).'</pre></td>';
            }
            $html .= '<tr>';
                $html .= '<td>'.$post_date.'</td>';
                $html .= '<td>'.$post->post_title.'</td>';
                $html .= $hasil_ujian;
                $html .= '<td class="text-center px-0">';
